...
function parseError(body) {
  ...
  var file = body.replace(/=*\n(.*?\.xml)(.|\n)*/, "$1");
  var i = body.indexOf("Error");
  var asError = body.substring(i).replace(/^\s+|\s+$/g,'');

  var error = {
    file: file
  };

  if (asError.match(/ID is already in use/)) {
    error.msg = "already imported or has conflicting id";
  } else {
    error.msg = asError;
  }
